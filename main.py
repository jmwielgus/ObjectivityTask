...
def load_file(base_dir: str) -> list:
    # Modify this
    with open(pt.Path('filedir/numbers.csv'),'r') as file:
        numbers_list=[]
        for line in file.readlines()[1:]:
            helper=[(float(line.replace('\n','').split(' ')[0])),
               (int(line.replace('\n','').split(' ')[1]))]          
            numbers_list.append(helper)   
    file.closed
    return numbers_list
...
def your_algorithm_executing_function(numbers_list) -> list:
    # And this
    my_results = mp.Pool(mp.cpu_count()).map(calculate_numbers, numbers_list)
    return my_results
